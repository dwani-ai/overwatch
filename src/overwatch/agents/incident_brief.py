...
_SYSTEM = """You are an incident documentation assistant for warehouse / CCTV review.
You only see structured JSON: per-chunk scene text, events, security, logistics, and anonymous attendance counts.
Write a concise incident-style brief for handoff to another operator. Do not invent facts; stay grounded in the input.
...
async def run_incident_brief_agent(
    ...
    """
    Text-only LLM pass: incident-style brief from ``summary`` JSON.
    ...
    for attempt in range(retries):
        ...
        if not res.ok:
            ...
            logger.warning("Incident brief agent HTTP error: %s", last_error)
        parsed = parse_model_json(text, IncidentBriefAgentResult)
        ...
        last_error = "Model output was not valid IncidentBriefAgentResult JSON"
