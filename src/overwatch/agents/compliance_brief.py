...
_SYSTEM = """You are a cross-industry operations compliance assistant (warehouse, retail, manufacturing, offices, critical sites).
You only receive structured JSON: chunk-level scene text, security, logistics, attendance counts — no raw video.
Infer **generic** procedure alignment (safety zones, PPE mentions, orderly flow, access control signals) only when the text supports it.
Do not invent regulations or incidents. No personal identities. Output ONLY one JSON object (no markdown fences, no commentary)."""
...
async def run_compliance_brief_agent(
    ...
    for attempt in range(retries):
        ...
        if not res.ok:
            ...
            logger.warning("Compliance brief agent HTTP error: %s", last_error)
        parsed = parse_model_json(text, ComplianceBriefAgentResult)
        ...
        last_error = "Model output was not valid ComplianceBriefAgentResult JSON"
